...
const Footer = () => {
    ...
    return (
        <>
            ...
            <Box height="12" color='white' className={chatParent?.status === "Pending" ? `m-auto bg-blue-100 mb-12 flex justify-center items-center fixed inset-x-0 bottom-0` : chatParent?.status === "Proses" ? `m-auto bg-green-300 mb-12 flex justify-center items-center fixed inset-x-0 bottom-0` : `m-auto bg-slate-300 mb-12 flex justify-center items-center fixed inset-x-0 bottom-0`}>
                    <div className='grid grid-cols-1'>
                        {
                            chatParent?.status === "Pending" ?
                                <div className='grid grid-rows-1' onClick={() => placeOrder({ status: 'Proses' })}>
                                    <p className='text-md text-black justify-self-center flex'>Terima Pesanan <HiClipboardCheck className='ml-2' size={22} /></p>
                                </div> : chatParent?.status === "Proses" ?
                                    <>
                                        <div className='grid grid-rows-1' onClick={() => placeOrder({ status: 'Selesai' })}>
                                            <p className='text-xl text-black justify-self-center'>Selesaikan Pesanan</p>
                                        </div>
                                    </> : <div className='grid grid-rows-1'>
                                        <p className='text-xl text-black justify-self-center'>Pesanan Selesai</p>
                                    </div>
                        }
                    </div>
                </Box>
