...
export const getAllUsers = async (req: Request, res: Response) => {
    try {
        const response: any = await supabase.from('users').select('id, username, email, phone_number, profile_pict, address, role, description, created_at').order('id', { ascending: true });
...
export const updateUserProfile = async (req: any, res: Response) => {
    try {
        if (req.params.id) {
            const response: any = await supabase.from('users').update({ username: req.body.username, description: req.body.description, address: req.body.address, phone_number: req.body.phone_number }).eq('id', req.params.id).select('id, username, email, phone_number, profile_pict, address, role, description, province, state, city, created_at')
